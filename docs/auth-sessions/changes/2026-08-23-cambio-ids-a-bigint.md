...
El id de `refresh_tokens` (y de las demás tablas del módulo `auth`) pasa de `uuid` a
`bigint GENERATED ALWAYS AS IDENTITY`. Incluye también `user_id` y `company_id` como `bigint`
(antes `uuid`).
...
priorizó `bigint` por ser más liviano y más simple de depurar. `token_hash` sigue siendo un
valor opaco de alta entropía — el cambio no lo afecta.

## Qué había antes

`id uuid NOT NULL` como llave primaria y `user_id`, `company_id` como FKs `uuid`.
