...
El id de `role_menu_permissions` (y de las demás tablas del módulo `auth`) pasa de `uuid` a
`bigint GENERATED ALWAYS AS IDENTITY`. Incluye también `role_id` y `menu_id` como `bigint`
(antes `uuid`).
...
`id uuid NOT NULL` como llave primaria y `role_id`, `menu_id` como FKs `uuid`.
