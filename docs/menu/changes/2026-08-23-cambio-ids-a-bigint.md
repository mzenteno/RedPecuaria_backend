...
El id de `menus` (y de las demás tablas del módulo `auth`) pasa de `uuid` a
`bigint GENERATED ALWAYS AS IDENTITY`. Incluye también `parent_id` como `bigint` (antes
`uuid`).
...
priorizó `bigint` por ser más liviano y más simple de depurar. La `key` sigue siendo el
identificador estable usado por el sistema de permisos, no se ve afectada por este cambio.

## Qué había antes

`id uuid NOT NULL` como llave primaria y `parent_id uuid` como FK auto-referenciada.
