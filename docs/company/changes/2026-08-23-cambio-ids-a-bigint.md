...
El id de `companies` (y de las demás tablas del módulo `auth`) pasa de `uuid` (generado en el
dominio) a `bigint GENERATED ALWAYS AS IDENTITY` (generado por Postgres al insertar).
...
`id uuid NOT NULL` como llave primaria, sin generación por parte de la base de datos.
