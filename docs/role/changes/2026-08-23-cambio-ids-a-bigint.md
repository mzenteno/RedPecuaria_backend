...
El id de `roles` (y de las demás tablas del módulo `auth`) pasa de `uuid` (generado en el
dominio) a `bigint GENERATED ALWAYS AS IDENTITY` (generado por Postgres al insertar). Incluye
también `company_id` como `bigint` (antes `uuid`).
...
`id uuid NOT NULL` como llave primaria y `company_id uuid NOT NULL` como FK.
